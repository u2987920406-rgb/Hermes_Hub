import { ArrowLeft, FolderOpen, Play, Save } from 'lucide-react'
import { useCallback, useEffect, useMemo, useState } from 'react'
import { PageHeader } from '../components/PageHeader'
import { api } from '../lib/api'
import { useHubStore } from '../store/useHubStore'
import { FILE_HINTS, STANDARD_FILES, statusLabels } from '../types'
...
export function ProjectDetail({ projectId, onBack, onMenu }: Props) {
  ...
  const switchFile = (next: string) => {
    if (dirty && !window.confirm('Des modifications non enregistrees seront perdues. Continuer ?')) {
      return
    }
    setFile(next)
  }
        ...
        <div className="flex flex-1 flex-col overflow-hidden p-4 sm:p-6">
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            <p className="text-xs muted">
              ...
              {dirty && <span className="ml-2 text-amber-600 dark:text-amber-400">- non enregistre</span>}
            </p>
            <button onClick={save} className="btn-primary px-3 py-1.5 text-xs" disabled={!dirty || saving}>
              <Save className="h-3.5 w-3.5" />
              {saving ? 'Enregistrement...' : 'Enregistrer'}
              <kbd className="ml-1 hidden rounded bg-white/20 px-1 text-[10px] sm:inline">Ctrl+S</kbd>
            </button>
          </div>
